...
;(function($){
	...
	var pd = unsafeWindow.PageData;
	var kw = pd.forum.forum_name;	
	var fid = pd.forum.forum_id;
	var tbs = pd.tbs;	
	var tid = "";
	pd.hasOwnProperty("thread") && (tid = pd.thread.thread_id);
	...
	function gui(){
		$("#tb_rich_poster .poster_submit:first").before("<select id='os_slt'></select>\
		<a id='client_btn' style='margin:0 10px;' class='ui_btn ui_btn_m j_submit poster_submit'>\
		<span class='poster_submit'><em>客户端</em></span></a>");
		var osSelect = "";
		var OS = ["iPhone","Android","WindowsPhone","Windows 8"];
		OS.map(function(os,i){osSelect += "<option value='"+i+"'>"+os+"</option>";});
		$("#os_slt").css({"float":"left","height":"27.5px","width":"110px","font-size":"17px"}).append(osSelect);
		if(tid){
			$("#os_slt")[0].addEventListener("change",replyPost,false);  //帖子内页
			$("#client_btn")[0].addEventListener("click",replyPost,false);
			lzl();
		}
		else{								
			$("#os_slt")[0].addEventListener("change",threadPost,false); //主题页
			$("#client_btn")[0].addEventListener("click",threadPost,false); 
		}
	}
	...
	function format(content){
		...
		.replace(/&nbsp;/g, " ").replace(/([^/]+?)\.png/ig,function($1) { //表情
			if (smiley[$1]){
				return ">" + smiley[$1] + "<";
			}
		}) 
	...
	function lzlCallback(response) {
		...
		if(msg.error_code === "0"){
			...
			$(".lzl_simple_wrapper .tb-editor-editarea").empty();
		}
		else{
			errorMsg(msg.error_msg);
		}
	...
	function threadPost(){
		...
		var postURL = "http://c.tieba.baidu.com/c/c/thread/add";
		clientPost(postURL,data,postCallback);
	}
		
	function replyPost(){
		if($("#ueditor_replace").html() === "<p><br></p>" || $("#ueditor_replace").html() === ""){
			$('.poster_error').text("回复内容不能为空");
			return;
		}
		imgFormat();
		...
		var postURL = "http://c.tieba.baidu.com/c/c/post/add";
